...
class MB_API_Post {
  ...
  // Note:
  //   MB_API_Post objects must be instantiated within The Loop.
  ...
  function MB_API_Post($wp_post = null) {
    if (!empty($wp_post)) {
  ...
  function save($values = null) {
    ...
    if (!empty($values['author'])) {
      $author = $mb_api->introspector->get_author_by_login($values['author']);
    ...
    if (isset($values['categories'])) {
      ...
      foreach ($categories as $category_slug) {
        ...
        $category = $mb_api->introspector->get_category_by_slug($category_slug);
    ...
    if (!empty($_FILES['attachment'])) {
      ...
      $this->attachments[] = new MB_API_Attachment($attachment_id);
  ...
  function import_wp_object($wp_post) {
    global $mb_api, $post;
    $date_format = $mb_api->query->date_format;
    ...
    $this->set_format_id_value();
  }
  
  function set_value($key, $value) {
    global $mb_api;
    if ($mb_api->include_value($key)) {
      $this->$key = $value;
    } else {
      unset($this->$key);
    }
  ...
  function set_categories_value() {
    global $mb_api;
    if ($mb_api->include_value('categories')) {
      $this->categories = array();
      if ($wp_categories = get_the_category($this->id)) {
        foreach ($wp_categories as $wp_category) {
          $category = new MB_API_Category($wp_category);
  ...
  function set_tags_value() {
    global $mb_api;
    if ($mb_api->include_value('tags')) {
      $this->tags = array();
      if ($wp_tags = get_the_tags($this->id)) {
        foreach ($wp_tags as $wp_tag) {
          $this->tags[] = new MB_API_Tag($wp_tag);
        }
      }
    } else {
      unset($this->tags);
  ...
  function set_author_value($author_id) {
    global $mb_api;
    if ($mb_api->include_value('author')) {
      $this->author = new MB_API_Author($author_id);
    } else {
      unset($this->author);
    }
  }
  
  function set_format_id_value() {
    global $mb_api;
    if ($mb_api->include_value('format')) {
      $this->format_id = get_post_format($this->id);
    } else {
      unset($this->format_id);
    }
  }
  
  function set_comments_value() {
    global $mb_api;
    if ($mb_api->include_value('comments')) {
      $this->comments = $mb_api->introspector->get_comments($this->id);
    } else {
      unset($this->comments);
    }
  }
  ...
  function set_attachments_value() {
    global $mb_api;
    if ($mb_api->include_value('attachments')) {
      $this->attachments = $mb_api->introspector->get_attachments($this->id);
    } else {
  ...
  function set_thumbnail_value() {
    global $mb_api;
    if (!$mb_api->include_value('thumbnail') ||
  ...
  function set_custom_fields_value() {
    global $mb_api;
    if ($mb_api->include_value('custom_fields') &&
        $mb_api->query->custom_fields) {
      $keys = explode(',', $mb_api->query->custom_fields);
  ...
  function get_thumbnail_size() {
    global $mb_api;
    if ($mb_api->query->thumbnail_size) {
      return $mb_api->query->thumbnail_size;
