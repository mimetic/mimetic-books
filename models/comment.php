...
class MB_API_Comment {
  ...
  function MB_API_Comment($wp_comment = null) {
    if ($wp_comment) {
  ...
  function import_wp_object($wp_comment) {
    ...
    if (!empty($wp_comment->user_id)) {
      $this->author = new MB_API_Author($wp_comment->user_id);
    } else {
      unset($this->author);
    }
  }
  ...
  function comment_id_not_found() {
    global $mb_api;
    $mb_api->error("Post ID '{$_REQUEST['post_id']}' not found.");
  }
  
  function comment_closed() {
    global $mb_api;
    $mb_api->error("Post is closed for comments.");
  }
  
  function comment_on_draft() {
    global $mb_api;
    $mb_api->error("You cannot comment on unpublished posts.");
  }
  
  function comment_post_redirect() {
    global $comment, $mb_api;
    $status = ($comment->comment_approved) ? 'ok' : 'pending';
    $new_comment = new MB_API_Comment($comment);
    $mb_api->response->respond($new_comment, $status);
  }
