...
class MB_API_Author {
  ...
  // Note:
  //   MB_API_Author objects can include additional values by using the
  //   author_meta query var.
  
  function MB_API_Author($id = null) {
    if ($id) {
      $this->id = (int) $id;
    } else {
  ...
  function set_author_meta() {
    ...
    $vars = explode(',', $mb_api->query->author_meta);
