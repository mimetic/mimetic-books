...
class MB_API_Query {
  ...
  function get_controller() {
    ...
    if (preg_match('/^[a-zA-Z_]+$/', $mb)) {
      return $this->get_legacy_controller($mb);
    } else if (preg_match('/^([a-zA-Z_]+)(\/|\.)[a-zA-Z_]+$/', $mb, $matches)) {
      return $matches[1];
    } else {
      return 'core';
    }
  ...
  function get_legacy_controller($mb) {
    global $mb_api;
    if ($mb == 'submit_comment') {
      if ($mb_api->controller_is_active('respond')) {
        return 'respond';
      } else {
        $mb_api->error("The 'submit_comment' method has been removed from the Core controller. To use this method you must enable the Respond controller from WP Admin > Settings > Mimetic Books API.");
      }
    } else if ($mb == 'create_post') {
      if ($mb_api->controller_is_active('posts')) {
        return 'posts';
      } else {
        $mb_api->error("The 'create_post' method has been removed from the Core controller. To use this method you must enable the Posts controller from WP Admin > Settings > Mimetic Books API.");
      }
    } else {
      return 'core';
    }
  ...
  function get_method($controller) {
    ...
    //
    // Note:
    //   The implicit outcome (3) is invoked by setting the mb query var to a
    //   non-empty value on any WordPress page:
    //     * http://example.org/2009/11/10/hello-world/?mb=1 (get_post)
    //     * http://example.org/2009/11/?mb=1 (get_date_posts)
    //     * http://example.org/category/foo?mb=1 (get_category_posts)
    ...
    if (empty($method)) {
      ...
      return false;
    } else if (method_exists("MB_API_{$controller}_Controller", $method)) {
      ...
      if (is_search()) {
        ...
        if (empty($_GET['mb'])) {
          $mb_api->error("Uknown method '$method'.");
        }
