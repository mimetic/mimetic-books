...
class MB_API_Introspector {
	...
	// Get the post and convert to the MB_API_Post format.
	public function get_post($post_id) {
		global $post;
		
		$post = get_post($post_id);
		$p = new MB_API_Post($post);
		return $p;
	}
	...
	public function get_categories($key) {
		...
		foreach ($wp_categories as $wp_category) {
			...
			if ($key) {
				$categories[$wp_category->$key] = $this->get_category_object($wp_category);
			} else {
	...
	public function get_current_category() {
		global $mb_api;
		extract($mb_api->query->get(array('id', 'slug', 'category_id', 'category_slug')));
		if ($id || $category_id) {
			if (!$id) {
				$id = $category_id;
			}
	...
	public function get_current_tag() {
		global $mb_api;
		extract($mb_api->query->get(array('id', 'slug', 'tag_id', 'tag_slug')));
		if ($id || $tag_id) {
			if (!$id) {
				$id = $tag_id;
			}
	...
	public function get_current_author() {
		global $mb_api;
		extract($mb_api->query->get(array('id', 'slug', 'author_id', 'author_slug')));
		if ($id || $author_id) {
			if (!$id) {
				$id = $author_id;
			}
	...
	public function get_comments($post_id) {
		...
		foreach ($wp_comments as $wp_comment) {
			$comments[] = new MB_API_Comment($wp_comment);
		}
		return $comments;
	}
	...
	public function get_attachments($post_id) {
		...
		if (!empty($wp_attachments)) {
			foreach ($wp_attachments as $wp_attachment) {
				$attachments[] = new MB_API_Attachment($wp_attachment);
			}
		}
		return $attachments;
	}
	...
	public function attach_child_posts(&$post) {
		...
		foreach ($wp_children as $wp_post) {
			$post->children[] = new MB_API_Post($wp_post);
		}
	...
	protected function get_category_object($wp_category) {
		...
		return new MB_API_Category($wp_category);
	}
