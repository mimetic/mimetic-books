...
class MB_API_Response {
  
  function setup() {
    global $mb_api;
    $this->include_values = array();
    $this->exclude_values = array();
    if ($mb_api->query->include) {
      $this->include_values = explode(',', $mb_api->query->include);
    }
    // Props to ikesyo for submitting a fix!
    if ($mb_api->query->exclude) {
      $this->exclude_values = explode(',', $mb_api->query->exclude);
  ...
  function get_json($data, $status = 'ok') {
    ...
    $data = apply_filters('mb_api_encode', $data);
  ...
  function respond($result, $status = 'ok') {
    ...
    } else if ($mb_api->query->redirect) {
      $url = $this->add_status_query_var($mb_api->query->redirect, $status);
      wp_redirect($url);
    } else if ($mb_api->query->callback) {
      // Run a JSONP-style callback with the result
      $this->callback($mb_api->query->callback, $json);
    } else {
      // Output the result
      $this->output($json);
    }
